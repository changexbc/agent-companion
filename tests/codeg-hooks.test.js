...
for(const marker of [{parent:214,kind:'delegate'},{parent:214,kind:'regular'},{parent:null,kind:'delegate'}]) {
 test(`child events are ignored using metadata ${JSON.stringify(marker)}`,async t=>{
  ...
  hub.ingest({source:'codex',sessionId:'thr-child',type:'start',roundId:'r1',ts:Date.now(),cwd:'/project/test'});
  hub.ingest({source:'codex',sessionId:'thr-child',type:'wait',roundId:'r1',ts:Date.now()+1,callId:'ask',text:'Child question'});
  for(const name of CODEG_EVENTS)assert.equal(await h.ingestHook(event(name,{connection_id:'child'})),true);
  assert.equal(hub.sessions.size,2);
  assert.deepEqual(hub.snapshot().sessions.map(s=>s.id),['codeg:214']);
  assert.ok(hub.snapshot().events.every(e=>e.sessionId!=='codex:thr-child'));
  ...
  assert.equal(hub.sessions.size,2,'API failure never resurrects a known child');

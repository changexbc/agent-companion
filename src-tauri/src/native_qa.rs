    ...
    std::thread::spawn(move || {
        ...
        for (label, window) in app.webview_windows() {
            ...
            let _ = window.with_webview(move |platform| {
                ...
                let script = NSString::from_str(r#"JSON.stringify({title:document.title,connection:document.querySelector('.desktop-connection')?.dataset.connection,avatars:document.querySelectorAll('.desktop-avatar').length,wait:document.querySelectorAll('.desktop-avatar[data-status=wait]').length,text:document.body.innerText,menu:[...document.querySelectorAll('[role=menuitem]')].map(e=>e.textContent),settingsReady:document.querySelector('fieldset')?.disabled===false,resources:performance.getEntriesByType('resource').map(r=>r.name),width:innerWidth,height:innerHeight})"#);

...
// Analytic poses allow deterministic scrubbing. No per-frame physics integration or timers.
  ...
  function curve(points, closed = true, tightness = .13) {
    let d = `M${points[0].map(f).join(' ')}`;
    const count = points.length;
    for (let i = 0; i < count - (closed ? 0 : 1); i++) {
      const at = n => points[closed ? (n + count) % count : Math.max(0,Math.min(count-1,n))];
  ...
  function sampleContour(points, count = N) {
  ...
  function railPoint(u, rect = {x:720,y:181,width:64,height:251}) {
  ...
  function leadPoint(u, project = p => p) {
  ...
  function releasePoint(tail,u, project = p => p, scale = 1) {
    if(u>=.6) return leadPoint((u-.6)/.4,project);
    const v=u/.6, a=tail, b=[tail[0]-45*scale,tail[1]+10*scale], c=project([578,410]), d=leadPoint(0,project);
    return [0,1].map(k=>(1-v)**3*a[k]+3*(1-v)**2*v*b[k]+3*(1-v)*v*v*c[k]+v**3*d[k]);
  }
  function step(q, amplitude) {
    q = ((q % 1) + 1) % 1;
  ...
  function blink(t, at, width = .09) { return Math.max(.08, 1 - .96 * Math.exp(-(((t-at)/width)**4))); }
  function pose(t, study) {
  ...
  function createLineCat(svg, options = {}) {
    ...
    const project=([x,y])=>[rect.x+(x-720)*size,base+(y-400)*size];
    function render(t, study=false) {
      ...
      far.setAttribute('opacity',study?.62:farAlpha*.62);
      ...
      ].map(pts=>curve(pts.map(p.transform),true,.13)).join(' '));
      ...
      eyes.setAttribute('opacity',study?'1':f(1-smooth((t-4.5)/.28)));
      line.setAttribute('opacity',study?'1':f(1-smooth((t-6.65)/.35)));
      svg.setAttribute('data-time',f(t));
      svg.setAttribute('data-gather',f(gather));
      svg.setAttribute('data-draw',f(draw));
      return {gather, draw, points:current};
    }
    return {render};
  };
export { createLineCat };
